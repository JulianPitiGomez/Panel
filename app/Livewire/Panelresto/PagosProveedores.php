...
class PagosProveedores extends Component
{
    use WithPagination;
    ...
    public function cargarDatosIniciales()
    {
        ...
        // Cargar cajas (obtener las distintas cajas de la tabla ordpag)
        $this->cajas = DB::connection('client_db')
            ->table($this->tablePrefix . 'ordpag')
            ->select('caja')
            ->distinct()
            ->whereNotNull('caja')
            ->orderBy('caja')
    ...
    public function getPagosProperty()
    {
        ...
        $query = DB::connection('client_db')
            ->table($this->tablePrefix . 'ordpag as p')
            ->leftJoin($this->tablePrefix . 'provee as c', 'p.proveedor', '=', 'c.codigo')
            ->select(
                'p.NUMERO',
                'p.PROVEEDOR as CLIENTE',
        ...
        if ($this->clienteSeleccionado) {
            $query->where('p.PROVEEDOR', $this->clienteSeleccionado);
        }
    ...
    public function calcularResumenFormasPago()
    {
        ...
        $this->resumenFormasPago = DB::connection('client_db')
            ->table($this->tablePrefix . 'ordcon as pc')
    ...
    {
        return view('panelresto.pagos-proveedores', [
            'pagos' => $this->pagos
        ]);
    }
