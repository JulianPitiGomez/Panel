        ...
        <main>
                ...
                <div class="w-full bg-[#1E1B35] flex justify-between items-center px-4 py-2">
                    ...
                    <div x-data="{ showInfo: false }" class="relative">
                        <div @click="showInfo = !showInfo"
                            ...
                            {{ strtoupper(substr(session('vendedor_nombre'), 0, 1)) . strtoupper(substr(session('vendedor_nombre'), 1, 1)) }}
                        </div>
                        ...
                        <div x-show="showInfo" @click.away="showInfo = false"
                            ...
                            <div class="p-4 space-y-2">
                                <div>
                                    ...
                                    {{ session('vendedor_nombre') }}<br>
                                    <span class="text-xs">Código: {{ session('vendedor_user_id') }}</span>
                                </div>
                                <hr>
                                <div>
            ...
            <div class="fixed bottom-0 left-0 w-full bg-white border-t shadow-inner flex justify-around py-2 text-xs text-gray-600">
                ...
                <a href="{{route('historial')}}" wire:navigate>
                    <div class="flex flex-col items-center {{ request()->routeIs('historial') ? 'text-yellow-500' : '' }}">
                        <i class="fa fa-history text-lg"></i>
                        <span class="text-[10px]">Historial</span>
                    </div>
                </a>
                <a href="{{route('listas-precios')}}" wire:navigate>
                    <div class="flex flex-col items-center {{ request()->routeIs('listas-precios') ? 'text-yellow-500' : '' }}">
                        <i class="fa fa-list text-lg"></i>
                        <span class="text-[10px]">Lista</span>
                    </div>
                </a>
                <a href="{{route('deudas')}}" wire:navigate>
                    <div class="flex flex-col items-center {{ request()->routeIs('deudas') ? 'text-yellow-500' : '' }}">
                        <i class="fa fa-dollar-sign text-lg"></i>
                        <span class="text-[10px]">A cobrar</span>
                    </div>
                </a>
                <a href="{{route('logout.vendedores')}}" wire:navigate>
