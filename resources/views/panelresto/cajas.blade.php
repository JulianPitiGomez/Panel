...
<div class="grid grid-cols-2 lg:grid-cols-4 gap-2 md:gap-4">
        <div class="bg-white rounded-lg md:rounded-xl shadow-sm border border-gray-200 p-3 md:p-4">
            <div class="flex items-center">
                ...
                <div class="ml-2 md:ml-4 min-w-0">
                    <p class="text-xs md:text-sm font-secondary font-medium text-gray-600">Cajas</p>
                    <p class="text-lg md:text-2xl font-primary font-bold text-gray-900 truncate">{{ number_format($totalCajas) }}</p>
                </div>
            </div>
        </div>
        ...
        <div class="bg-white rounded-lg md:rounded-xl shadow-sm border border-gray-200 p-3 md:p-4">
            <div class="flex items-center">
                ...
                <div class="ml-2 md:ml-4 min-w-0">
                    <p class="text-xs md:text-sm font-secondary font-medium text-gray-600">Faltantes</p>
                    <p class="text-lg md:text-2xl font-primary font-bold text-red-600 truncate">${{ number_format($totalFaltantes, 2) }}</p>
                </div>
            </div>
        </div>
        ...
        <div class="bg-white rounded-lg md:rounded-xl shadow-sm border border-gray-200 p-3 md:p-4">
            <div class="flex items-center">
                ...
                <div class="ml-2 md:ml-4 min-w-0">
                    <p class="text-xs md:text-sm font-secondary font-medium text-gray-600">Sobrantes</p>
                    <p class="text-lg md:text-2xl font-primary font-bold text-blue-600 truncate">${{ number_format($totalSobrantes, 2) }}</p>
                </div>
            </div>
        </div>
        ...
        <div class="bg-white rounded-lg md:rounded-xl shadow-sm border border-gray-200 p-3 md:p-4">
            <div class="flex items-center">
                ...
                <div class="ml-2 md:ml-4 min-w-0">
                    <p class="text-xs md:text-sm font-secondary font-medium text-gray-600">Efectivo</p>
                    <p class="text-lg md:text-2xl font-primary font-bold text-green-600 truncate">${{ number_format($totalEfectivo, 2) }}</p>
                </div>
            </div>
        </div>
    </div>
...
<div class="flex flex-col lg:flex-row gap-6">
    <!-- Contenido Principal -->
    <div class="{{ $mostrarPanel ? 'hidden lg:block lg:w-2/3' : 'w-full' }} transition-all duration-300 overflow-hidden">

    <!-- Tabla/Lista de Cajas -->
    <div class="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
        ...
        <div class="px-3 md:px-6 py-3 md:py-4 border-b border-gray-200 bg-gray-50">
            <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 md:gap-4">
                ...
                @if($cajas->hasPages())
                <div class="hidden sm:block text-xs md:text-sm font-secondary text-gray-700">
                    {{ $cajas->firstItem() }} - {{ $cajas->lastItem() }} de {{ $cajas->total() }}
                </div>
                @endif
            </div>
        </div>
        ...
        <div class="lg:hidden divide-y divide-gray-200">
            ...
            <div class="p-3 hover:bg-gray-50 transition-colors duration-200">
                <div class="flex justify-between items-start mb-2">
                    <div>
                        ...
                        <div class="text-xs font-secondary text-gray-600">
                            {{ \Carbon\Carbon::parse($caja->fecha)->format('d/m/Y') }}
                            @if($caja->hora)
                                - {{ \Carbon\Carbon::parse($caja->hora)->format('H:i') }}
                            @endif
                        </div>
                    </div>
                ...
                <div class="text-xs font-secondary {{ $this->getClaseSaldo($caja->saldo) }} mb-2">
                    {{ $this->getTipoSaldo($caja->saldo) }}
                </div>
        ...
        <div class="px-3 md:px-6 py-3 md:py-4 border-t border-gray-200 bg-gray-50">
            <div class="flex flex-col sm:flex-row items-center justify-between gap-3">
                <div class="text-xs md:text-sm font-secondary text-gray-700 text-center sm:text-left">
                    {{ $cajas->firstItem() }} - {{ $cajas->lastItem() }} de {{ $cajas->total() }}
                </div>
        ...
        <div class="text-center py-8 md:py-12">
            <i class="fas fa-cash-register text-gray-400 text-4xl md:text-5xl mb-4"></i>
            <h3 class="text-base md:text-lg font-primary font-medium text-gray-900 mb-2">No hay cajas</h3>
            <p class="text-sm md:text-base text-gray-600 font-secondary">No se encontraron turnos con los filtros seleccionados</p>
        </div>
        @endif
    </div>
    ...
    @if($mostrarPanel && $cajaSeleccionada)
            ...
            <div class="space-y-2 md:space-y-3">
                ...
                @if($detallesCaja && count($detallesCaja) > 0)
                    ...
                    <div class="bg-white border border-gray-200 rounded-lg p-2 md:p-3 hover:shadow-sm transition-shadow duration-200">
                        ...
                        <div class="grid grid-cols-2 gap-1 md:gap-2 text-xs">
                            @if($detalle->debe > 0)
                    ...
                    <div class="bg-gray-100 border border-gray-300 rounded-lg p-2 md:p-3 mt-3 md:mt-4">
                        <h5 class="text-xs md:text-sm font-secondary font-bold text-gray-900 mb-2">TOTALES GENERALES</h5>
